...
int main()
{
	...
	pauseMessage.setOrigin(pauseRect.left + pauseRect.width/2.0f, pauseRect.top + pauseRect.height/2.0f);
    ...
    while (window.isOpen())
    {
        ...
        if (isPlaying)
        {
			...
			if (sf::Touch::isDown(0))
			{
				...
				if (relativePos.y / touchscale > leftPaddle.getPosition().y)
					touchdir = 1;
				else
					touchdir = -1;							
				maxmove = relativePos.y / touchscale - leftPaddle.getPosition().y;
			}						
            ...
            {
                isPlaying = false;
                pauseMessage.setString("You lost!\nTouch screen to restart or\npress back to exit");
				pauseRect = pauseMessage.getLocalBounds();
				pauseMessage.setOrigin(pauseRect.left + pauseRect.width/2.0f, pauseRect.top + pauseRect.height/2.0f);
				pauseMessage.setPosition(gameWidth / 2, gameHeight / 2);
            }
