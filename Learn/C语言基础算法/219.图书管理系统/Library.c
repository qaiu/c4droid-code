   ...
   void showmenu(char **menu,int count,int sr,int sc)
    {
       ...
       for(i=0;i<count;i++)
	{
	  gotoxy(sc,sr++);  //textcolor(DARKGRAY);
	   printf(menu[i]);
	}
    }
...
/***********************Funtion To Get User's Choice*************************/
  int getchoice(char **menu,char *hotkeys,int r,int c,int count)
  {
   ...
   char *s;
     while(1)
      {
	gotoxy(c,r+choice-1);  //textcolor(WHITE+BLINK); printf(menu[choice-1]);
	while(!kbhit())
	;
	ascii=getch();
	if(ascii==0)
	...
	gotoxy(c,r+choice-1);  //textcolor(DARKGRAY);    printf(menu[choice-1]);
...
/*********************Function To Add New Members****************************/
  void addmember()
  {
	...
	d = *localtime(&times);
//     getdate(&d);
     M.ms.year=d.tm_year;     M.ms.day=d.tm_mday;     M.ms.mon=d.tm_mon;
...
/*************Function To Record Transaction Made By Members*****************/
   ...
   void transac1(int mid,char ch)
   {
     ...
     T.b.bno=0;  T.b.gno=0; T.b.no=0;
     	time_t times = time(NULL);
	d = *localtime(&times);
     T.i.day=d.tm_mday;  T.i.mon=d.tm_mon;  T.i.year=d.tm_year;
...
void issbook()
{
   ...
   int f1,f2;
   clrscr();
   gotoxy(5,1); printf("Enter Book id:");
   gotoxy(9,2); printf("/");gotoxy(13,2); printf("/");
...
void transac2(int mid,int bno,int gno,int no,char c)
{
	...
	d = *localtime(&timer);
	 ...
	 {
	   T.r.day=d.tm_mday; T.r.mon=d.tm_mon; T.r.year=d.tm_year;
	   T.reason='R';
 ...
 while(fread(&T,sizeof(T),1,ft)==1)
  {
   ...
   printf("E");
   //textcolor(DARKGRAY);
   printf("-Member Renewed ");
   //textcolor(WHITE);
   printf("D");
   //textcolor(DARKGRAY);
   printf("-Duplicate I card ");
   //textcolor(WHITE);
   printf("I");
   //textcolor(DARKGRAY);
   printf("-Book Issued ");
...
void showhelp()
{
	   ...
	   gotoxy(9,20);
	   //textcolor(LIGHTMAGENTA);
	   printf("Send your comments/queries to ");
	   //textcolor(LIGHTGREEN+BLINK);
	   printf("obaid786@sancharnet.in");
	   getch();
}